    ...
    for ($y = 0; $y < $height; $y++) {
        ...
        for ($x = 1; $x <= 7; $x++) {
            ...
            if (look($currDay, $dates)) {
                ...
                echo "<p>" . ucwords($loc) . "</p>"; //makes first letter in each word uppercase
            }
            echo "</td>";
        }
        echo "<tr>";
...
function getDates($max, $numDays) {
    $days = array(); // holds the days to travel on
    for ($i = 0; $i < $numDays; $i++) {
        $day = rand(1, $max);
        while (look($day, $days)) { //loops over $days array until each each $day is different
            $day = rand(1, $max);
        }
        array_push($days, $day); //pushes $day into back of $days array
    }
    return $days;
}

function getLocations($max, $locations, $order) { //num_location, country, order
    if ($order == 1)
        sort($locations);
    else if ($order == 2)
...
function validate(&$logs) {
    ...
    if ($errorFree) { //array_push inserts elements at the end of the array
